...
V112: Routing correcto + Sync CrearPSL + Audit limpio
...
def perfil_crm(tel):
    ...
    if imo_rows:
        ...
        for r in imo_rows:
            ...
            nom = r.get("Nombre","")
            eq  = r.get("Equipo","")
            nom_fmt = formatear_nombre_empatia(f"{ape} {nom}")
            pend_list.append(f"• {nom_fmt} ({eq})")
        p["pendientes"] = pend_list
    elif px_row:
        p["tipo"]       = "PX"
    ...
    if p["tipo"] == "PX":
        ...
        nums = [int(m.group()) for eq in equipos for m in [re.search(r"\d+",eq)] if m]
    ...
    if st == "MAIN":
        if up in {"1","2","3"} and s.get("caso_followup"):
            ...
            if up == "1":
                ...
                wa(JOSE_TEL, f"✅ *Caso cerrado* por {nom_full}\nPX: wa.me/{tel_caso}", f"SIS→JOSE")
            elif up == "2":
                ...
                wa(JOSE_TEL, f"🆘 *{nom_full} necesita apoyo*\nCaso: wa.me/{tel_caso}", f"SIS→JOSE")
        ...
        if up == "1":
            cerrar_caso(tel_caso, f"Resuelto por {nom_full}")
            wa(tel, f"✅ *Caso cerrado correctamente.*\nRegistrado. Gerencia notificada. 👏", f"SIS→{nom}")
            wa(JOSE_TEL,
            ...
            wa(tel, f"🟡 *Caso → En Gestión.*\nRecuerda actualizarlo cuando lo cierres.", f"SIS→{nom}")
        elif up == "3":
            actualizar_caso(tel_caso, "ABIERTO", f"Sin respuesta — {nom_full} reporta")
            wa(tel, f"🔴 *Sin contacto registrado.*\nGerencia notificada para apoyo.", f"SIS→{nom}")
            wa(JOSE_TEL,
            ...
            wa(tel, f"Responde: 1️⃣ Resolví  2️⃣ En proceso  3️⃣ Sin contacto  4️⃣ Nota  0️⃣ Volver", f"SIS→{nom}")
            return
        ...
        wa(tel, f"📝 *Nota registrada en el caso.*\n\n0️⃣ Menú | 4️⃣ Volver a mis casos", f"SIS→{nom}")
        s["st_cc"] = "VER_CASOS"; set_s(tel, s)
        return
...
def _limpiar_input(txt):
    """Limpia caracteres basura de inputs numéricos."""
...
def api_enviar():
    ...
    msg = d.get("mensaje", d.get("texto","")).strip()
...
def api_bienvenida_progreso():
    ...
    try:
        ...
        logger.error(f"bienvenida_progreso: {e}")
        return jsonify({"error": str(e), "filas": [], "total": 0}), 500
...
def reporte_consolidado():
    ...
    return jsonify({
        ...
        "reportes":    len(_reportes_hoy) if _REP_OK else 0,
...
def panel():
    try:
        ...
        return "<h2>Panel no disponible</h2>",200

# ══════════════════════════════════════════════════════════════
# CARGA DE MÓDULOS DEL SISTEMA
# ══════════════════════════════════════════════════════════════
# NOTA IMPORTANTE — AUDIT v112:
# Los try/except de carga de módulos están claramente separados.
# El bloque de Sync CrearPSL se carga UNA SOLA VEZ al final del archivo,
# antes de if __name__=="__main__". No mezclar bloques.
...
# Importar worker de seguimiento (GitHub o Autonomo)
# AUDIT v112: bloque LIMPIO — try/except correctamente cerrado, sin contaminación
try:
...
def seg_reenvio():
    ...
    try:
        ...
        return jsonify({"error":"run_reenvio no disponible"}), 503
    d   = request.json or {}
    res = run_reenvio(
...
def seg_detectar():
    ...
    try:
        ...
        return jsonify({"error":"detectar_sin_respuesta no disponible"}), 503
...
def _flujo_gerente(tel, up, texto):
    ...
    if st == "PEGAR_REPORTE":
        ...
        if cc_detectada == "DESCONOCIDA":
            ...
            set_s(tel, s)
            wa(tel,
               f"🤖 *No pude detectar de qué coordinadora es.*\n\n"
    ...
    if up == "7S":
        ...
        wa(tel, "📤 Bienvenida E27 iniciada — 50 mensajes en cola.", "GERENTE")
        s["st_jose"] = "MENU"; set_s(tel, s)
        return
...
def _keepalive_loop():
    ...
    INTERVALO = 23 * 3600
    time.sleep(3600)  # primera ejecución después de 1h
    while True:
        try:
...
def api_bienvenida_preview():
    ...
    msg = (
        ...
        f"🏨 Hotel José Antonio Deluxe, Miraflores\n\n"
...
def refresh_crm_cache(force=False):
    """Refresca la caché de datos del CRM desde Google Sheets."""
    now = time.time()
    ...
    try:
        ...
        if not df.empty:
            ...
            logger.info("Caché de CRM refrescada ✅")
        return df, otros
    except Exception as e:
...
def api_chat():
    ...
    try:
        ...
        if len(msg_norm) > 3 and df is not None:
            ...
            if matches:
                search_results = "\nHe encontrado estos registros en la base de datos:\n" + "\n".join(matches)
        contexto = (
            "Eres el Cerebro de CPSL (🔱), asistente de IA estratégico para el CRM de Crear Lima. "
            "Estás entrenado para responder a Gerentes (José), Coordinadoras (Diana, Joyce, Zuley) y CMJ. "
            f"📊 ESTADO: {stats_txt}\n"
            f"{search_results}"
        )
...
def api_debug_logs():
    try:
        log_path_d = os.path.join(DATA_DIR, "bot.log")
        if not os.path.exists(log_path_d):
            return "Log file not found", 404
        with open(log_path_d, "r", encoding="utf-8") as f:
            lines = f.readlines()
            return "<pre>" + "".join(lines[-200:]) + "</pre>"
    except Exception as e:
        return str(e), 500

# ── SCHEDULER IMOs (lazy start) ──────────────────────────────
_scheduler_started = False
_scheduler_lock = threading.Lock()

def _scheduler_imos():
    """Refresco IMOs y recordatorios diarios."""
    ya_enviado_hoy = set()
    while True:
        try:
            hora = ahora().strftime("%H:%M")
            ...
            if hora == hora_imos and clave_imo not in ya_enviado_hoy:
                ...
                logger.info(f"[IMO-SCHED] Enviando mensajes principales ({hora_imos})")
                _enviar_mensajes_imos()
            clave_rec = f"{fecha}-rec"
            if hora == hora_recordatorios and clave_rec not in ya_enviado_hoy:
                ya_enviado_hoy.add(clave_rec)
                logger.info(f"[RECORDATORIOS] Enviando recordatorios ({hora_recordatorios})")
                _disparar_recordatorios_imos()
        except Exception as e:
            logger.error(f"[IMO-SCHED] {e}")
        time.sleep(60)
...
def _disparar_recordatorios_imos():
    logger.info("🚀 Iniciando envío de recordatorios a IMOs...")
    try:
        from seguimiento_imos import enviar_recordatorios_imos
        from sync_cloud import conectar_sheets
        c = conectar_sheets()
...
def start_background_tasks_once():
    """Inicia el scheduler de IMOs de forma segura en la primera petición."""
...
# ══════════════════════════════════════════════════════════════
# SINCRONIZADOR CrearPSL Global — UN SOLO BLOQUE, AL FINAL
# AUDIT v112: este es el ÚNICO bloque del sync. No duplicar arriba.
# ══════════════════════════════════════════════════════════════
try:
...
if __name__ == "__main__":
    logger.info("🚀 CPSL Torre de Control V112 + Sync CrearPSL")
