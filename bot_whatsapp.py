    ...
    def enviar_mensaje(telefono, texto, nombre_mostrar="", registrar_sheets=False, mensaje_usuario=""):
        ...
        try:
            ...
            if r.status_code == 200:
                ...
                if registrar_sheets:
                    estado = "EMBUDO" if "NUEVO CONTACTO" in nombre_mostrar or "CONTACTO" in nombre_mostrar else "MENÚ INTERACTIVO"
                    threading.Thread(
                        target=registrar_en_sheets, 
                        args=(telefono, nombre_mostrar, mensaje_usuario, texto[:500], estado),
                        daemon=True
                    ).start()
                return True
            else:
...
def enviar_mensaje(telefono, texto, nombre_imo="", registrar_sheets=False, msg_user=""):
    ...
    if sesion.get("primera_vez", True) and not str(nombre_imo).startswith("COORDINADORA"):
        aclaracion = (
            "\n\n🤖 _Nota: Estás comunicándote con *IA Cuántica*. "
            "Mis respuestas pueden ser limitadas. Para más información, comunícate con nuestras coordinadoras:_\n\n" + COORDINADORAS
        )
...
class GoogleSheetsAPI:
    ...
    def get_token(cls):
        ...
        with cls._token_lock:
            ...
            try:
                ...
                logger.error(f"Error generando JWT Sheets: {e}")
            return None
    ...
    def leer_sheet(cls):
        ...
        try:
            ...
            logger.error(f"Error leyendo de Sheets: {e}")
        return []
...
def embudo_ventas_ia(mensaje_usuario, nombre_conocido=None, nombre_ya_saludado=False):
    cfg = {
        ...
        "modo_ia": Config.MODO_IA,
        "ia_primaria": Config.IA_PRIMARIA,
        "ia_fallback": Config.IA_FALLBACK,
    }
    ...
    def construir_prompt_qwen():
        return [
            {"role": "system", "content": """Eres un Asesor Experto de 'Crear Poder Sin Límites Perú'. 
            Responde usando SOLO el brochure oficial. Eres humano, directo y empático. Máximo 3 oraciones."""},
    ...
    def llamar_qwen():
        ...
        try:
            response = Generation.call(
                model="qwen-turbo", 
                messages=construir_prompt_qwen(),
                api_key=cfg["dashscope_key"],
                result_format="message", max_tokens=500, temperature=0.7
            )
    ...
    modelo_primario = "qwen" if modo == "ab_test" and hash(mensaje_usuario) % 2 == 0 else cfg.get("ia_primaria", "qwen")
    ...
    if not respuesta:
        logger.warning(f"Modelo {modelo_primario} falló ({error}). Intentando fallback...")
        respuesta, error = llamar_gemini() if modelo_fallback == "gemini" else llamar_qwen()
    
    if not respuesta:
        logger.warning(f"Ambas IAs fallaron. Usando banco local.")
        return respuesta_del_banco(mensaje_usuario)
...
def procesar_mensaje(telefono, texto, imo_nombre_completo):
    ...
    if sesion.get("menu_state") == "esperando_encuesta":
        if texto_limpio in ["1", "2", "3", "4", "5"]:
            threading.Thread(
                target=registrar_en_sheets, 
                args=(telefono, nombre_mostrar, "Calificación del Asistente", f"{texto_limpio} Estrellas", "ENCUESTA CSAT"), 
                daemon=True
            ).start()
