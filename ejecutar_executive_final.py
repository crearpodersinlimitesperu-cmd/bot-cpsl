...
def enviar_executive_v6_rocio_final():
    print("--- INICIANDO DESPACHO DEFINITIVO EXECUTIVE v6.0 (ROCÍO) ---")
    db = SessionLocal()
    ...
    # 3. Adjunto ÚNICO: Contrato Executive v6.0 (Blindaje Total)
    ...
    try:
        ...
        print("   [OK] Correo Executive v6.0 enviado a ROCÍO.")
        ...
        t = TrazabilidadPX(
            px_id=px_id,
            canal="EMAIL",
            tipo_evento="ENVIO_EXECUTIVE_V6_FINAL",
            contenido="Bienvenida Suprema Executive v6.0 (Contrato Diligenciado) enviada exitosamente.",
            metadatos='{"asunto": "EXECUTIVE SUPREMA C1 E28", "version": "6.0"}'
        )
        db.add(t)
        ...
        log = LogEnvio(
            ...
            tipo="OUT",
            canal="EMAIL",
            mensaje="Bienvenida Executive v6.0 C1 E28",
            status_code=200
        )
        db.add(log)
        db.commit()
        ...
        print(f"   [ERR] Error en el despacho Executive Final: {e}")
        db.rollback()
    finally:
        db.close()

if __name__ == "__main__":
    enviar_executive_v6_rocio_final()
