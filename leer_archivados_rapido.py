...
try:
    ...
    # Buscar solo correos desde 1 de mayo y que NO sean nuestros (para que sea rápido)
    status, messages = mail.search(None, f'(SINCE "01-May-2026" NOT FROM "{GMAIL_USER}")')
    if status == 'OK' and messages[0]:
        email_ids = messages[0].split()
        print(f"Total correos en Archivados/Todos (no enviados por nosotros): {len(email_ids)}")
        encontrados = []
