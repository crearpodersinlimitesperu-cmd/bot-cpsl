...
def fetch_bounces():
    ...
    # Buscar correos de Mail Delivery Subsystem de hoy
    date_str = date.today().strftime("%d-%b-%Y")
    status, messages = mail.search(None, f'(FROM "mailer-daemon@googlemail.com" SINCE "{date_str}")')
...
print("1. Conectando a IMAP para buscar rebotes recientes...")
bounces = fetch_bounces()
print(f"Encontrados {len(bounces)} correos rebotados en total hoy.")
...
for imo_dni, pxs in imos_afectados.items():
    ...
    # Buscar datos del IMO en la BD (buscamos en campo identificacion o nombre)
    ...
    # Preparar texto de participantes rebotados
    lista_px_html = ""
    lista_px_sms = ""
    for p in pxs:
    ...
    if imo_email and "@" in str(imo_email):
        try:
            ...
            <html>
                <body style="font-family: Arial, sans-serif; color: #333;">
                    ...
                    <p>Acabamos de enviar la campaa de bienvenida del <strong>Captulo 1</strong> (Equipo 28) a los participantes bajo tu enrolamiento. Sin embargo, hemos detectado que <strong>los correos de los siguientes participantes han rebotado</strong> (son invǭlidos o estǭn mal escritos):</p>
                    <ul>{lista_px_html}</ul>
                    <p>Para asegurar que no se pierdan ningǧn detalle del evento, te pedimos por favor que te comuniques con ellos y nos respondas a este correo con sus <strong>direcciones de email correctas y actualizadas</strong>.</p>
        ...
        print(f" -> No tiene email vǭlido registrado.")
