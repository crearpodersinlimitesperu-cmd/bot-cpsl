...
def limpiar_telefono(tel_str):
    if pd.isna(tel_str) or str(tel_str).strip() == 'nan':
        return ""
...
def cargar_imos_rebotes():
    """Carga IMOs cuyos correos están en la lista de rebotes."""
    with open(r'C:\Users\josem\Downloads\bot-cpsl-review\rebotes_recientes.txt', 'r', encoding='utf-8') as f:
        rebotes = set(line.strip().lower() for line in f if line.strip())
    ...
    # Filtrar IMOs cuyos correos rebotaron y tienen teléfono
    imo_rebotes = {}
    for k, v in imo_info.items():
        if v['correo'] and v['correo'].lower() in rebotes:
            if v['telefono'] and v['telefono'] != 'nan':
                imo_rebotes[k] = v
            else:
                print(f"IMO {v['nombre']} rebotó ({v['correo']}) pero no tiene teléfono registrado.")

    return imo_rebotes, df_aptos

def ejecutar():
    print("="*70)
    print("  ENVÍO DE SMS A IMOS REBOTADOS - APTOS E25/E26/E27")
    print("="*70)

    imos_rebotes, df_aptos = cargar_imos_rebotes()
    print(f"\nIMOs rebotados con teléfono a contactar: {len(imos_rebotes)}")

    enviados = 0
    errores = 0
    omitidos = 0

    log = open(r'C:\Users\josem\Downloads\bot-cpsl-review\reporte_sms_imos_rebotes.txt', 'w', encoding='utf-8')
    log.write("REPORTE DE SMS A IMOS REBOTADOS - APTOS E25/E26/E27\n")
    log.write("="*70 + "\n\n")

    for imo_id, info in sorted(imos_rebotes.items()):
        ...
        mensaje = (
            f"Hola {nombre_imo}, de CREAR PSL. (Te escribimos por aqui porque tu correo rebotó). "
            ...
            f"Responde LISTO cuando confirmen."
        )
    ...
    print(f"{'='*70}")
    print("Reporte: reporte_sms_imos_rebotes.txt")
