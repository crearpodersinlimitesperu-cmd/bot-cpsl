...
def enviar_forense_v9_seguro():
    print("--- INICIANDO DESPACHO FORENSE v9.0 (FIDELIDAD TOTAL - UNICO ADJUNTO) ---")
    db = SessionLocal()
    ...
    # 3. Adjunto ÚNICO: Contrato Forense v9.0 (Protocolo Literal de Fuente .doc)
    file_path = r"C:\Users\josem\Downloads\bot-cpsl-review\Contrato_Forense_ROCIO_JARA.pdf"
    if os.path.exists(file_path):
        try:
            with open(file_path, "rb") as attachment:
                ...
                msg.attach(part)
                print("   [OK] Contrato Forense v9.0 adjuntado exitosamente.")
    ...
    try:
        ...
        print("   [OK] Correo Forense v9.0 Final enviado a Jose Sanchez.")

        # 5. Registrar en Caja Negra (Inmortalización del Protocolo Legal)
        t = TrazabilidadPX(
            px_id=0,
            canal="EMAIL",
            tipo_evento="ENVIO_FORENSE_V9_FINAL",
            contenido="Despacho Forense v9.0 (FIDELIDAD TOTAL) basado en documento .doc oficial. Sin simplificaciones.",
            metadatos='{"asunto": "FORENSE SUPREMA", "version": "9.0", "fuente": "doc_oficial"}'
        )
        db.add(t)
        db.commit()
        print("   [OK] Registro maestro legal guardado en la Caja Negra Pro.")
