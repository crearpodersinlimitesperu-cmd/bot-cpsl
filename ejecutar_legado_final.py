...
def enviar_legado_v5_rocio_final():
    print("--- INICIANDO DESPACHO DEFINITIVO LEGADO v5.0 (ROCÍO) ---")
    db = SessionLocal()
    ...
    # 3. Adjunto ÚNICO: Contrato Legado v5.0 (Blindaje Total)
    ...
    try:
        ...
        print("   [OK] Correo Legado v5.0 enviado a ROCÍO.")
        ...
        t = TrazabilidadPX(
            px_id=px_id,
            canal="EMAIL",
            tipo_evento="ENVIO_LEGADO_V5_FINAL",
            contenido="Bienvenida Suprema Legado v5.0 (Contrato Diligenciado) enviada exitosamente.",
            metadatos='{"asunto": "LEGADO SUPREMO C1 E28", "version": "5.0"}'
        )
        db.add(t)
        ...
        log = LogEnvio(
            ...
            tipo="OUT",
            canal="EMAIL",
            mensaje="Bienvenida Legado v5.0 C1 E28",
            status_code=200
        )
        db.add(log)
        db.commit()
        ...
        print(f"   [ERR] Error en el despacho Legado Final: {e}")
        db.rollback()
    finally:
        db.close()

if __name__ == "__main__":
    enviar_legado_v5_rocio_final()
