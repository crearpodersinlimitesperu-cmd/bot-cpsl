...
def enviar_integro_v8_seguro():
    print("--- INICIANDO DESPACHO INTEGRA v8.0 (FIDELIDAD ABSOLUTA - UNICO ADJUNTO) ---")
    db = SessionLocal()
    ...
    # 3. Adjunto ÚNICO: Contrato Integra (Seguridad Máxima)
    ...
    try:
        ...
        print("   [OK] Correo Integra v8.0 enviado a Jose Sanchez.")

        # 5. Registrar en Caja Negra
        t = TrazabilidadPX(
            px_id=0,
            canal="EMAIL",
            tipo_evento="ENVIO_INTEGRA_V8",
            contenido="Prueba Integra v8.0 (FIDELIDAD ABSOLUTA) con UNICO adjunto PDF diligenciado para Jose Sanchez.",
            metadatos='{"asunto": "INTEGRA SUPREMA", "version": "8.0"}'
        )
        db.add(t)
        db.commit()
        print("   [OK] Prueba Integra v8.0 registrada en la Caja Negra Pro.")
